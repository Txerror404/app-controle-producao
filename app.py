...
with aba1:
    with st.container(border=True):
        ...
        # Garantir que produtos estejam carregados
        if 'df_produtos' not in st.session_state:
            st.session_state.df_produtos = carregar_produtos_google()
        ...
        with c1:
            ...
            # Selectbox de ID_ITEM
            lista_itens = sorted(df_prod['id_item'].unique().tolist())
            item_sel = st.selectbox("📌 Selecione o ID_ITEM", lista_itens, key="item_lanc")

            # Buscar produto selecionado
            produto_info = df_prod[df_prod['id_item'] == str(item_sel).strip()]

            if not produto_info.empty:
                info = produto_info.iloc[0]
                descricao_texto = str(info.get('descricao', 'N/A'))
                cliente_texto = str(info.get('cliente', 'N/A'))

                try:
                    carga_sugerida = int(float(info.get('qtd_carga', CARGA_UNIDADE)))
                except:
        ...
        data_ini = c4.date_input(
            "📅 Data de Início",
            sugestao_h.date(),
            key="data_lanc"
        )

        hora_ini = c5.time_input(
            "⏰ Hora de Início",
            sugestao_h.time(),
            key="hora_lanc"
        )
