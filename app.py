...
with st.sidebar:
    ...
    if not df_exp.empty:
        ...
        st.download_button("📥 Baixar Produção", csv, "pcp_william.csv", "text/csv", key="side_dl")
    if st.button("Sair"):
        st.session_state.auth_ok = False
        st.rerun()
...
with aba5:
    ...
    if not df_dash.empty:
        ...
        for i, m in enumerate(MAQUINAS):
            ...
            c_m[i].metric(label=f"Carga {m}", value=f"{hrs:.1f} h")
