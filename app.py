...
# =================================================================
st.set_page_config(page_title="PCP Industrial v5.0", layout="wide")
...
# CSS para UI Personalizada (Gráfico e Abas)
st.markdown("""
    <style>
        .block-container {padding-top: 0.5rem;}
        .modebar-container { top: 0 !important; }
        .stTabs [data-baseweb="tab-list"] { gap: 8px; }
        .stTabs [data-baseweb="tab"] { background-color: #1e1e1e; border-radius: 4px; padding: 4px 12px; color: white; }
        .stTabs [aria-selected="true"] { background-color: #FF4B4B !important; }
    </style>
""", unsafe_allow_html=True)

# =================================================================
# 2. BANCO DE DADOS E FUNÇÕES DE APOIO
# =================================================================
def conectar(): 
...
st.markdown(f"""
    <div style="background-color: #1E1E1E; padding: 10px 15px; border-radius: 8px; border-left: 8px solid #FF4B4B; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center;">
        <div>
            <h2 style="color: white; margin: 0; font-size: 20px;">📊 PCP <span style="color: #FF4B4B;">|</span> CRONOGRAMA</h2>
        ...
        <div style="text-align: right; border: 1px solid #FF4B4B; padding: 2px 12px; border-radius: 5px; background-color: #0E1117;">
            <h3 style="color: #FF4B4B; margin: 0; font-size: 18px;">⏰ {agora.strftime('%H:%M:%S')}</h3>
        </div>
    </div>
    """, unsafe_allow_html=True)

# =================================================================
# 4. FUNÇÃO DO GRÁFICO (LAYOUT MARCADO)
# =================================================================
def plotar_gantt(lista_maquinas, height_grafico=500):
    ...
    if not df_all.empty:
        ...
        if not df_g.empty:
            ...
            # Layout conforme suas marcações (Linhas horizontais e Barras próximas)
            ...
            fig.update_xaxes(type='date', range=[agora - timedelta(hours=2), agora + timedelta(hours=48)], dtick=10800000, tickformat="%H:%M\n%d/%m")
            
            # AGORA e Linha Vermelha
            ...
            fig.update_layout(height=height_grafico, margin=dict(l=10, r=10, t=50, b=100), bargap=0.01, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
...
with aba1:
    with st.container(border=True):
        ...
        qtd = c3.number_input("📊 Qtd", min_value=1, value=int(info.get('qtd_carga', CARGA_UNIDADE)) if id_item_sel else int(CARGA_UNIDADE))
            ...
            if op_num and id_item_sel:
                ...
                with conectar() as conn:
                    ...
                    cur.execute("INSERT INTO agenda (maquina, pedido, item, inicio, fim, status, qtd) VALUES (?,?,?,?,?,?,?)", 
                               (maquina_sel, f"{info.get('cliente','N/A')} | {op_num}", id_item_sel, inicio.strftime('%Y-%m-%d %H:%M:%S'), fim_prod.strftime('%Y-%m-%d %H:%M:%S'), "Pendente", qtd))
                    if setup_min > 0:
                        conn.execute("INSERT INTO agenda (maquina, pedido, item, inicio, fim, status, qtd, vinculo_id) VALUES (?,?,?,?,?,?,?,?)", 
                                    (maquina_sel, f"SETUP {op_num}", "Ajuste", fim_prod.strftime('%Y-%m-%d %H:%M:%S'), (fim_prod + timedelta(minutes=setup_min)).strftime('%Y-%m-%d %H:%M:%S'), "Setup", 0, cur.lastrowid))
                    conn.commit()
                st.rerun()
    ...
    if not df_ger.empty:
        ...
        for _, prod in df_ger[df_ger["status"].isin(["Pendente", "Setup"])].sort_values("inicio").iterrows():
            with st.expander(f"📌 {prod['maquina']} | {prod['pedido']}"):
                ...
                # REPROGRAMAÇÃO (DATA E HORA)
                if is_admin:
                    n_data = col1.date_input("Mudar Data", prod['inicio'].date(), key=f"d_{prod['id']}")
                    n_hora = col2.time_input("Mudar Hora", prod['inicio'].time(), key=f"t_{prod['id']}")
                ...
                # BOTÕES DE AÇÃO
                if col3.button("✅ Concluir", key=f"ok_{prod['id']}", use_container_width=True):
...
st.divider()
st.caption(f"PCP v5.0 | Refresh 2min | {agora.strftime('%H:%M:%S')}")
